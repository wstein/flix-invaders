...
Always use the `./flix` wrapper, not a `flix` on `PATH`: it downloads the compiler
version pinned in `flix.toml`, re-verifies it against the SHA-256 committed in
`.flix-wrapper/lock.toml` on every run, and caches it by content -- so local runs match CI
...
- `flix`, `flix.cmd`, `.flix-wrapper/` — the bootstrap wrapper; generated by flixw, but
  **committed on purpose** so a clone needs only Java. Repair them with `./flix doctor
  --fix` and move to a newer flixw with `./flix wrapper --upgrade`, never by hand; `./flix
...
- **`exitActual` is Processing's only `System.exit(0)`**, and `flix run` does not fork a JVM.
  ...
  alive. `./flix run` hides this because its launcher owns the JVM; a plain `java -cp ...
...
- **`build-jar` packages stale classes, without bound.** Flix emits a class per closure and
  ...
  of them in the artifact, which took ten minutes and produced a corrupt 1.5GB jar. `./flix
