...
| Scoring the bot's target instead of taking the lowest invader | Proposed as a real strategic tradeoff — clear the lowest row, narrow the block, prefer targets you can shoot on the way — and it is a real tradeoff *in the arcade original*. Built and measured over ten seeds: plain lowest-row averages **level 4.5**; adding a flank bonus drops it to 4.1, a travel penalty to 4.4, and an "invaders passed en route" bonus to **1.1**, because the most distant target has the most on the way to it, so the cannon is dragged across the field and never clears the bottom row. Re-read as local density it recovers to 3.9 and still loses. Two reasons it cannot pay here: `Rules.marchSpeedFor` keys off the *count* of survivors rather than the block's width, so narrowing has no mechanical payoff; and `Demo.worthFiring` already shoots whatever is overhead, so the bot harvests on the way regardless of what it is aiming at. Making flanking worthwhile is a change to the **rules**, not to the bot. |
