...
| Scoring the bot's target instead of taking the lowest invader | Proposed as a strategic tradeoff — clear the lowest row, or narrow the block so the formation marches further before it turns. **The narrowing half is real and large**: `Game.liveBounds` measures the formation from its *living* invaders, so emptying an outer column genuinely widens the runway. Measured, a bot weighted to do it cuts level 1 from 5-6 row drops per 1000 ticks to **3** — it nearly halves the descent. It still loses. Over ten seeds, plain lowest-row averages level **4.5**; fixed flank weights give 3.8-4.2, and weighting flanks toward the start of a level, when there is a level left to spend the time on, at best *ties* at 4.5. The reason is that descent is not the binding constraint: deaths are unchanged (18 against 19 across six seeds), and what narrowing costs is kill *throughput* — fewer kills, lower score, fewer bonus lives, a shorter game, a lower level. **In this game the clock that runs out is lives, not the invasion line.** Two traps if anyone revisits it: the flank bonus must be divided by how many invaders are still stacked in the column, because killing one of five moves no edge at all and rewards a strategy that never completes; and an "invaders passed en route" bonus is catastrophic (level 1.1), since the most distant target has the most on the way to it and the cannon ends up dragged across the field. Note also that `Demo.worthFiring` already shoots whatever is overhead, so the bot harvests as it walks whatever it is aiming at. |
